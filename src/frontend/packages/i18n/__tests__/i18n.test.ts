...
describe('integration testing on i18n package', () => {
  ...
  test('cmd extract-translation:main', () => {
    // To be sure the file is not here
    fs.rmSync('./locales/main/translations-crowdin.json', {
      recursive: true,
      force: true,
    });
    expect(
      fs.existsSync('./locales/main/translations-crowdin.json'),
    ).toBeFalsy();

    // Generate the file
    execSync('yarn extract-translation:main');
    expect(
      fs.existsSync('./locales/main/translations-crowdin.json'),
    ).toBeTruthy();
  });
