...
class FolderCreator:
    ...
    def get_workspace_path(self, workspace):
        return f"/tmp/workspace_{workspace.id}"
    ...
    def __create_folder(self, current_dir: str, workspace: Workspace, folder: OsmoseFolder):
    ...
    def __download_folder_files(self, folder: OsmoseFolder, path: str):
        ...
        for file in folder.files:
            download_url = os.path.join(settings.OSMOSE_BASE_ENDPOINT, file.raw_data["downloadUrl"])
            destination = os.path.join(path, file.name + os.path.splitext(file.raw_data["originalFilename"])[1])
