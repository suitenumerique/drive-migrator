    ...
    <div className="container container--center">
      {isSynchronizing ? (
        <>
          ...
          <p>{t("Nous récuperons les informations de votre compte Osmose ...")}</p>
            {isLong && <p>{t("Cela prend un peu plus de temps avec votre compte, merci de patienter cela peut prendre plusieurs minutes ...")}</p>}
        </>
      ) : (
        <>
          <h1>{t('Synchronisation réussie !')}</h1>
          <p>{t("Nous avons bien récupéré les informations liées à votre compte Osmose !")}</p>
          <Button href="/dashboard" className="mt-s">{t('Consulter mes communautés')}</Button>
        </>
      )}
    </div>
  );
