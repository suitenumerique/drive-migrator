...
def test_upload_to_s3_puts_file_content():
    ...
    mock_requests.put.assert_called_once_with(policy_url, data=file_content)


def test_notify_upload_ended_calls_endpoint(settings):
    """notify_upload_ended() calls POST /api/v1.0/items/{id}/upload-ended/."""
    settings.DRIVE_API_BASE_URL = "https://drive.example.com"
...
def test_find_user_by_email_returns_user_when_found(settings):
    """find_user_by_email() returns the user dict when Drive finds a match."""
    settings.DRIVE_API_BASE_URL = "https://drive.example.com"
...
def test_share_with_user_posts_to_accesses_endpoint(settings):
    ...
    mock_requests.post.assert_called_once_with(
        ...
        json={"user_id": "user-uuid", "role": "editor"},
        headers={"Authorization": "Bearer tok"},
    )
